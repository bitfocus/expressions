...
/**
 * Build the set of expression functions.
 *
 * `defaultTimezone` is used as the effective timezone for the date/time extraction functions
 * (`dateYear`..`dateWeekday`, `dateFormat`) when the caller does not pass an explicit `tz` argument.
 */
...
const expressionFunctionsCache = new Map<string | undefined, Record<string, (...args: any[]) => any>>()

/**
 * Get the set of expression functions, with date/time functions defaulting to the given timezone when
 * no explicit `tz` argument is passed (callers without a timezone context should pass `undefined`, which
 * uses the process-local timezone).
 *
 * The result is memoized per timezone. `executeExpression` is a hot path (runs for every expression option
 * and feedback evaluation), and the timezone is effectively constant, so this keeps us building the function
 * set once-per-timezone rather than rebuilding ~60 closures on every call.
 *
 * Note: when adding new functions, make sure to update the docs!
 *
 * @param defaultTimezone IANA timezone name, or undefined/empty to use the process-local timezone
 */
export function createExpressionFunctions(
	defaultTimezone: string | undefined
): Record<string, (...args: any[]) => any> {
	const key = defaultTimezone || undefined
	let functions = expressionFunctionsCache.get(key)
	if (!functions) {
		functions = buildExpressionFunctions(key)
		expressionFunctionsCache.set(key, functions)
	}
	return functions
}
